...
# for resuming from previous checkpoint, do a eval first
_C.TRAIN.EVAL_FIRST = False
...
# if true, will gather prediction and label before computing loss
# for loss that need the whole batch info like the VICReg
_C.TRAIN.GATHER_BEFORE_LOSS = False

# for contrastive learning, use MoCo v3
_C.TRAIN.USE_MOCO = False
# a fixed momentum
_C.TRAIN.MOCO_MOMENTUM = 0.99   # best in the MoCo v3 paper

# If True, use zero redundancy optimizer to reduce gpu memory
# https://pytorch.org/tutorials/recipes/zero_redundancy_optimizer.html
#_C.TRAIN.USE_ZERO_OPTIM = False
# set SOLVER.OPTIMIZING_METHOD to "zero_sgd"
...
_C.TEST.NO_LOG_CONFIG = False


# options to run the testing, and save some files

# enable result saving on the master process
_C.TEST.ENABLE_SAVE = False
...
# specific config for each dataset

_C.MODEL.MULTI_DATASETS = ["kinetics", "mmit", "activitynet"]
# the sampling  ratio for each dataset
_C.MODEL.MULTI_REPLICAS = [1, 1, 1]

_C.MODEL.MULTI_LOSS_FUNCS = ["soft_cross_entropy", "bce_logit", "soft_cross_entropy"]
_C.MODEL.MULTI_NUM_CLASSES = [700, 292, 200]
_C.MODEL.MULTI_HEAD_ACT = ["softmax", "sigmoid", "softmax"]
# bce loss could be 0.5 and ce loss is 5.0, so some balancing might be needed
_C.MODEL.MULTI_LOSS_WEIGHTS = [1.0, 10.0, 1.0]

# use 2-layer MLP classification head instead of a single linear layer
_C.MODEL.MULTI_USE_MLP = False

# path to each dataset's annotations
_C.MODEL.MULTI_PATH_TO_DATA_DIR = []
# path to each dataset's video path
_C.MODEL.MULTI_PATH_PREFIX = []

# for cross-dataset projection layer
_C.MODEL.MULTI_ADD_CROSS_PROJ = False
# this seems to be bad
_C.MODEL.MULTI_CROSS_PROJ_ADD_TO_PRED = False  # add the proj_pred to each dataset

# loss function to use for projected logits on the target dataset
"""
_LOSSES = {
...
_C.MODEL.MULTI_PROJ_LOSS_FUNC = "soft_cross_entropy"
# weight on the loss before adding to the target dataset head's loss
_C.MODEL.MULTI_PROJ_LOSS_WEIGHT = 1.0

# loss type for the projection layer parameters, L1/L0 or no regularization
_C.MODEL.MULTI_PROJ_SPARSITY_LOSS_TYPE = ""
_C.MODEL.MULTI_PROJ_SPARSITY_WEIGHT = 1e-4  # the reg term weight  # loss would be 1.*

# path to initial cross dataset projection matrix weights
_C.MODEL.LOAD_MULTI_PROJ_INIT_FILE = ""  # path to a lot of d1_d2.npy
_C.MODEL.MULTI_FIX_PROJ = False   # fix the projection layer, should be used with init

_C.MODEL.MULTI_PROJ_TRAIN_DIFF_LR = False  # use a different lr for proj layer
_C.MODEL.MULTI_PROJ_LR = 0.00001
_C.MODEL.MULTI_PROJ_MOMENTUM = 0.1


# use VICREG loss on the representation embedding
_C.MODEL.USE_VICREG_LOSS = False
_C.MODEL.VICREG_LOSS_WEIGHT = 0.01

# moco copy the heads
_C.MODEL.MULTI_USE_MOCO = False
_C.MODEL.MULTI_MOCO_MOMENTUM = 0.9

# for video-text model, we could ignore loading the text part if video features
# are ready
_C.MODEL.LOAD_VISUAL = True
...
# If True, use 2d patch, otherwise use 3d conv patch.
...
# uniformly sampling frames. for web_video_text
_C.DATA.UNIFORM_SAMPLE_FRAME = False
...
# # for decord  # this is not working yet, not faster than CPU and low GPU util
_C.DATA.DECODING_BACKEND_GPU_ENABLE = False
...
# junwei: for Kinetics, Mmit, Activitynet, skip some videos during validation to save time
_C.DATA.VAL_SKIP = 1

# junwei: for video-text dataset, our test set might be too big
_C.DATA.TEST_SKIP = 1

# junwei: for testing a model on different dataset, the num_class might be different
# will only be used if model_diff_data is True
_C.DATA.MODEL_DIFF_DATA = False
...
# whether display tqdm progress bar
_C.USE_TQDM = True
...
# dont use ROI, given the prop proposal, crop a cube proposal for classification
_C.DETECTION.USE_CUBE_PROP = False
...
# path to all the train/val videos, added by junwei,
# for training/testing directly loading frames from videos
_C.AVA.VIDEO_PATH = ""
_C.AVA.LOAD_FROM_VIDEO = False
...
# added by junwei, only show these categories' AP
_C.AVA.yewu_related = [
    64, 8, 29, 27, 54, 15, 57, 11, 3, 33, 2, 5, 20, 36, 66, 71, 76,
    # junwei: added 02/16/2022, run/jog, jump, walk, throw, hit an object
    10, 7, 14, 58, 34]

# add ava-kinetics data during training, so no filtering of ava frame
# only used for train set
_C.AVA.ADD_KINETICS = False

# val-test on ava-kinetics
_C.AVA.IS_TEST_ON_KINETICS = False

# a file containing the frame count of both AVA and AVA-kinetics videos in 30fps
_C.AVA.KINETICS_VIDEO_FRAME_COUNT = "avakinetics.frame_count.csv"
...
_C.AVA.USE_LABEL_SMOOTHING = False
_C.AVA.LABEL_SMOOTHING_EPS = 0.1
