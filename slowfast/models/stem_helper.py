...
class ResNetBasicStem(nn.Module):
    ...
    def _construct_stem(self, dim_in, dim_out, norm_module):
        ...
        # need to change this to maxpool2d for ONNX2TNN
        # https://github.com/Tencent/TNN/blob/master/doc/en/user/onnx2tnn_en.md#3-operator-support-and-usage-restrictions
        self.pool_layer = nn.MaxPool3d(
            kernel_size=[1, 3, 3], stride=[1, 2, 2], padding=[0, 1, 1]
        )

        self.pool_layer2d = nn.MaxPool2d(
    ...
    def forward(self, x):
        ...
        # junwei: for TNN, we will use this only during inference, for training and testing, we need 3D pool to be fast
        N, C, T, H, W = x.shape
        #k = self.pool_layer2d(x.view(-1, H, W)).view(N, C, T, H//2, W//2)
        #x = self.pool_layer(x)
        #print(torch.allclose(k, x))
        # inference/ONNX generating
        #x = self.pool_layer2d(x.view(-1, H, W)).view(N, C, T, H//2, W//2)
        # training/test
        x = self.pool_layer(x)
