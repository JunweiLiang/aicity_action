...
class Nonlocal(nn.Module):
    ...
    def forward(self, x):
        ...
        # junweil: 12/01/2021, remove einsum
        #theta_phi = torch.einsum("nct,ncp->ntp", (theta, phi))
        # equal to (N, TxHxW, C) * (N, C, TxHxW) => (N, TxHxW, TxHxW).
        #test_theta_phi = torch.matmul(theta.permute(0, 2, 1), phi)
        #print(torch.allclose(theta_phi, test_theta_phi))
        #print(theta_phi.size(), test_theta_phi.size())
        ...
        #test_theta_phi_g = torch.matmul(theta_phi, g.permute(0, 2, 1)).permute(0, 2, 1)
        #print(torch.allclose(theta_phi_g, test_theta_phi_g))
        theta_phi_g = torch.matmul(theta_phi, g.permute(0, 2, 1)).permute(0, 2, 1)
