...
class ObjectDetectionEvaluator(DetectionEvaluator):
    ...
    def evaluate(self, only_cat_ids=None):
        ...
        # evaluated over all 80 or 60 classes
        (
            per_class_ap,
            mean_ap,
            _,
        ...
        # only keeps only_cat_ids results as well as mean AP
        category_index = label_map_util.create_category_index(self._categories, only=only_cat_ids)
        mean_only = []
        for idx in range(per_class_ap.size):
            if idx + self._label_id_offset in category_index:
                ...
                mean_only.append(per_class_ap[idx])
        ...
        if only_cat_ids is not None:
            pascal_metrics["meanOver%dCategories" % len(mean_only)] = np.mean(mean_only)
...
class ObjectDetectionEvaluation(object):
    ...
    def evaluate(self):
        ...
        for class_index in range(self.num_class):
            # ignore the class with no ground truth
