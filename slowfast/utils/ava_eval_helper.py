...
def evaluate_ava(
    ...
    # the preds are unique boxes -> all the class scores
    # convert to one box -> one label scores  for each class
    ...
    # groundtruth[0] -> each keyframe -> boxes
    ...
    results = run_evaluation(categories, groundtruth, detections, excluded_keys, only_cat_ids)
...
def run_evaluation(
    categories, groundtruth, detections, excluded_keys, only_cat_ids=None, verbose=True
):
    ...
    # add image_key (video, keyframe) -> box + label info
    ...
    # also image_key (video, keyframe) -> box + label scores
    for image_key in boxes:
        if image_key in excluded_keys:
            """
            logging.info(
    ...
    #junwei_yewu_related = [64, 8, 29, 27, 54, 15, 57, 11, 3, 33, 2, 5, 20, 36]

    # only the ground truth image_key are used
    metrics = pascal_evaluator.evaluate(only_cat_ids=only_cat_ids)

    if du.is_master_proc():
        pprint.pprint(metrics, indent=2)
    # print for easy copy to sheet
    keys = sorted(list(metrics.keys()))
    headers = [k[-10:] for k in keys]
    vals = ["%.4f" % metrics[k] for k in keys]
    print(",".join(headers))
    print(",".join(vals))
    return metrics
