...
def all_gather(tensors):
    ...
    # the ith tensor in the list comes from rank i.
...
def all_gather_cat_self(tensors):
    """
    All gathers the provided tensors from all processes across machines.
    Args:
        tensors (list): tensors to perform all gather across all processes in
    ...
    # this function cat the original tensor into the gathered ones, so
    # that the gradient can flow back
    """
        ...
        if isinstance(tensor, dict):
            ...
            for k in tensor_dict:
                ...
                all_tensor = torch.cat(
                    tensor_placeholder[:rank]
                    + [tensor]
                    + tensor_placeholder[rank+1:],
                    dim=0
                )
                new_tensor_dict[k] = all_tensor
            output_tensor.append(new_tensor_dict)
        else:
            ...
            output_tensor.append(all_tensor)
...
# this is not used
# utils.multiprocessing already has these
...
def is_master_proc(num_gpus=None):
    ...
    """
    if num_gpus is None:
        num_gpus = get_world_size()  # num_gpu * num_machine
    if torch.distributed.is_initialized():
        # just one process on the main machine will be true
...
def get_world_size():
    ...
    if not dist.is_initialized():
        return 1
    # get from the current process group
    # slowfast.utils.multiprocessing -> init_process_group:
...
def all_gather_unaligned(data, group=None):
    ...
    if get_world_size() == 1:
        return [data]
    if group is None:
        group = _get_global_gloo_group()  # for nccl, new_group is created here
    ...
    # there it will gather all tensor from the group, and will block till all there
