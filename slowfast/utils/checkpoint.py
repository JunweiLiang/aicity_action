...
def inflate_weight(state_dict_2d, state_dict_3d):
    ...
    for k, v2d in state_dict_2d.items():
        ...
        if len(v2d.shape) == 4 and len(v3d.shape) == 5:
            ...
            assert v2d.shape[:2] == v3d.shape[:2]
            # repeat N times and scale down N
            v3d = (
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        if inflation:
            ...
            # specifically for conv2d to conv3d
            ...
            for k, v in pre_train_dict.items():
                if k in model_dict and v.size() == model_dict[k].size():
                    ...
                    code_model_size = None
                    if k in model_dict:
                        code_model_size = model_dict[k].shape
                    logger.info(
                        "%s: pkl size %s, code model size %s" % (k, v.shape, code_model_size))
            ...
            if not_load_layers:
                for k in not_load_layers:
                    logger.info("Network weights {} ({}) not loaded.".format(
                        k, model_dict[k].shape))
        ...
        if "epoch" in checkpoint.keys() and not epoch_reset:
            ...
            if scaler:
                # we might continue a checkpoint with mixed precision that previously is not
...
def load_train_checkpoint(cfg, model, optimizer, scaler=None):
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        start_epoch = checkpoint_epoch + 1  # checkpoint_epoch could be == -1
    else:
        start_epoch = 0
