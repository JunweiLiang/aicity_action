...
class Mmit(torch.utils.data.Dataset):
    """
    Moments-in-Time video loader. Construct the  video loader, then sample
    ...
    Note that MMit is multi-label, so the label is a list of [B, C]
    """
    ...
    def __init__(self, cfg, mode, num_retries=10):
        ...
        if self.use_multi_dataset:
            dataset_idx = self.cfg.MODEL.MULTI_DATASETS.index("mmit")
            ...
            #self.num_classes = self.cfg.MODEL.MULTI_NUM_CLASSES[dataset_idx]
        else:
    ...
    def _construct_loader(self):
        ...
        with pathmgr.open(path_to_file, "r") as f:
            for clip_idx, path_label in enumerate(f.read().splitlines()):
                ...
                # zero-indexed label
                labels = labels.split(",")
                label_arr = np.zeros((self.num_classes), dtype=np.float32)
                for label in labels:
        ...
        for i_try in range(self._num_retries):
            ...
            if self.aug:
                if self.cfg.AUG.NUM_SAMPLE > 1:
                    ...
                    if self.use_multi_dataset:
                        extra_data["dataset_name"] = ["mmit"] * self.cfg.AUG.NUM_SAMPLE
