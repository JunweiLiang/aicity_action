...
class Mit(torch.utils.data.Dataset):
    """
    moments-in-time version video loader. Construct the  video loader, then sample
    ...
    def __init__(self, cfg, mode, num_retries=10):
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for Moments-in-Time".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        if self.use_multi_dataset:
            dataset_idx = self.cfg.MODEL.MULTI_DATASETS.index("mit")
        ...
        logger.info("Constructing Moments-in-Time {}...".format(mode))
        self.val_skip = 1
    ...
    def _construct_loader(self):
        ...
        with pathmgr.open(path_to_file, "r") as f:
            for clip_idx, path_label in enumerate(f.read().splitlines()):
                ...
                # moments-in-time v2 is a multi-class dataset
        ...
        ), "Failed to load Moments-in-Time split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing Moments-in-Time dataloader (size: {}) from {}".format(
        ...
        for i_try in range(self._num_retries):
            ...
            if self.aug:
                if self.cfg.AUG.NUM_SAMPLE > 1:
                    ...
                    if self.use_multi_dataset:
                        extra_data["dataset_name"] = ["mit"] * self.cfg.AUG.NUM_SAMPLE
