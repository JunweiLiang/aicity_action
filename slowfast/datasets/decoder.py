...
def temporal_sampling(frames, start_idx, end_idx, num_samples):
    ...
    """
    # this will make sure to have num_samples
    # in case video is not fps 30, there may be less frames, so we
    # just uniformly duplicate some frames
    index = torch.linspace(start_idx, end_idx, num_samples)
    # this will make sure all index within range
...
def torchvision_get_frames(video_handle, frame_idxs):
    reader = io.VideoReader(video_handle, "video")
    reader_md = reader.get_metadata()
    fps = reader_md["video"]["fps"][0]


    # there might be repeated frames in the end
    frame_idxs_dict = {f: 1 for f in frame_idxs}
    ranked_unique_frame_idxs = sorted(list(frame_idxs_dict.keys()))

    ranked_unique_timestamps_and_fidxs = [(f/fps, f) for f in ranked_unique_frame_idxs]

    got_frames = {}  # fidx -> img
    for timestamp, fidx in ranked_unique_timestamps_and_fidxs:
        reader.seek(timestamp)
        got_frames[fidx] = next(reader)

    return_frames = [got_frames[fidx] for fidx in frame_idxs]
...
def decode(
    ...
    try:
        if backend == "pyav":
            ...
            frames, fps, decode_all_video = decord_decode(
                container,
                sampling_rate,
                num_frames,
                clip_idx,
