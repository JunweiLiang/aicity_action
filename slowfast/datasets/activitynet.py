...
class Activitynet(torch.utils.data.Dataset):
    """
    ActivityNet clip version video loader. Construct the  video loader, then sample
    ...
    def __init__(self, cfg, mode, num_retries=10):
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for ActivityNet".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        if self.use_multi_dataset:
            dataset_idx = self.cfg.MODEL.MULTI_DATASETS.index("activitynet")
        ...
        logger.info("Constructing ActivityNet {}...".format(mode))
        self.val_skip = 1
    ...
    def _construct_loader(self):
        ...
        with pathmgr.open(path_to_file, "r") as f:
            for clip_idx, path_label in enumerate(f.read().splitlines()):
                ...
                # activitynet has <1% multi-label, so we take only the first one
                label = label.split(",")[0]
        ...
        ), "Failed to load ActivityNet split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing ActivityNet dataloader (size: {}) from {}".format(
    ...
    def __getitem__(self, index):
        ...
        if self.use_multi_dataset:
            extra_data["dataset_name"] = "activitynet"
        ...
        for i_try in range(self._num_retries):
            ...
            if self.aug:
                if self.cfg.AUG.NUM_SAMPLE > 1:
                    ...
                    if self.use_multi_dataset:
                        extra_data["dataset_name"] = ["activitynet"] * self.cfg.AUG.NUM_SAMPLE
