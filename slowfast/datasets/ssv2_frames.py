...
class Ssv2_frames(torch.utils.data.Dataset):
    """
    ...
    def _construct_loader(self):
        ...
        with pathmgr.open(
            os.path.join(
                ...
                # qualcomm ai's labels are like this
                #"labels.json",
            ),
            "r",
        ) as f:
        ...
        self._video_names = []
        self._labels = []
        # for training, there is 168913 video
        for video in label_json:
            video_name = video["id"]
            template = video["template"]
            template = template.replace("[", "")
            template = template.replace("]", "")
            # get the label from 0-173
            label = int(label_dict[template])
            self._video_names.append(video_name)  # "id": '78687'
        ...
        # load the frame list for each video
        ...
        # assuming temporal clips are just 1
