...
def convert_to_one_hot(targets, num_classes, on_value=1.0, off_value=0.0):
    ...
    # [N, num_class] of off_value ->
    # Tensor.scatter_(dim, index, src, reduce=None)
    return torch.full(
        (batch_size, num_classes), off_value, device=targets.device
...
def mixup_target(target, num_classes, lam=1.0, smoothing=0.0):
    ...
    """
    # this is for multi-class (single gt class per sample)?
    # TODO: check the math for multi-label case
    off_value = smoothing / num_classes
    on_value = 1.0 - smoothing + off_value

    if len(target.shape) == 1:
        # target is [N], convert to [N, num_classes], and the gt class will be on_value
        # while others will be off_value
        # got [N, num_classes] of on_value/off_value
        ...
        target2 = convert_to_one_hot(
            target.flip(0),  # reversed order within the mini-batch dimension
            num_classes,
            on_value=on_value,
            off_value=off_value,
        )
    else:
        # target is already [N, num_classes] of 1/0

        target1 = target * on_value + (1 - target) * off_value

        target2 = target.flip(0) * on_value + (1 - target.flip(0)) * off_value
...
class MixUp:
    ...
    def __init__(
        ...
        # if multi_head model, x is the same, but labels will be
        # {dataset_name: [B] or [B, num_classes]}
    ...
    def __call__(self, x, target):
        ...
        # here x could be changed
        lam = self._mix_batch(x)

        if self.is_multi_head:
            # target should be {dataset_name: [B] or [B, num_classes]}
            # now x is already combined with flip() on the B dimension
            # we could just do the same for each of the target
            # the label_masking in the loss will take care of the rest
            for dataset_name in target:
                target[dataset_name] = mixup_target(
                    target[dataset_name], self.num_classes, lam,
